...
class AuditsController extends ActiveController {
    ...
    public $weekDays = ['1' => 'Monday', '2' => 'Tuesday', '3' => 'Wednesday', '4' => 'Thursday', '5' => 'Friday', '6' => 'Saturday'];
    ...
    public function actionAllAudits() {
        try {
            ...
            if ($user_id) {
                ...
                $result = $this->getAuditsByUserForAInterval($user_id, $status);
                $result_start = [];
    ...
    private function getAuditsByUserForAInterval($user_id, $status) {
        $cDay = date('w');
        $cDate = date('Y-m-d');
        $cHour = date('H');
        $checklistIds = ArrayHelper::getColumn(models\Checklists::find()->select(['checklist_id'])->where(['cl_frequency_duration' => $cDay, 'cl_frequency_value' => 3])->orWhere(['cl_frequency_value' => 1])->orWhere(['cl_frequency_value' => 2])->asArray()->all(), 'checklist_id');
       ...
       /* print_r($result);exit;
        $result = (new yii\db\Query())->select('CONCAT_WS(" ", u.`first_name`, u.`last_name`) as name,HOUR(sa.start_time),sa.deligation_user_id,c.name as location_name,h.hotel_name,d.department_name,sa.updated_at as audit_submitted_date,ck.cl_audit_type as audit_type,ck.cl_name as audit_name,CONCAT_WS(" ", au.`first_name`, au.`last_name`) as assignedby,ck.cl_audit_type as audit_type,a.deligation_flag,sa.audit_schedule_id as audit_id,sa.status,a.audit_name as parent,sa.audit_schedule_name as child,sa.deligation_status,sa.start_date,sa.end_date')
                ->from('{{%audits}} a')
                ...
                ])->andWhere(['AND', ['<=', 'HOUR(sa.sstart_time)', date('H')], ['ck.cl_frequency_value' => 1], ['<=', 'DATE(sa.start_date)', date('Y-m-d')], ['>=', 'DATE(sa.end_date)', date('Y-m-d')]])
                ->orWhere(['AND', ['IN', 'ck.checklist_id', $checklistIds], ['<=', 'DATE(sa.start_date)', date('Y-m-d')], ['>=', 'DATE(sa.end_date)', date('Y-m-d')]])
                //  ->orWhere(['IN', 'ck.checklist_id', $checklistIds])
                ->andWhere(['<=', 'DATE(sa.start_date)', date('Y-m-d')])
                  ->andWhere(['>=', 'DATE(sa.end_date)', date('Y-m-d')]) 
                ->andWhere([
                    'IN',
                    'sa.status',
                    $status
